...
async fn connect_tls(uri: &Uri) -> TlsStream<TcpStream> {
    ...
    connector.connect(domain, socket).await.expect("failed to connect to TLS stream")
}
...
pub async fn connect(uri: Uri) -> FragmentCollector<TokioIo<Upgraded>> {
    ...
    let req = Request::builder()
        .method(Method::GET)
        .uri(&uri)
        .header(HOST, uri.host().expect("host is required"))
